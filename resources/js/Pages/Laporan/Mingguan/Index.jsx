...
export default function Index({ auth, props, semester, id_semester, kelompok_usia, kelompok, filtering, dimensi, minggu_ke_mulai, minggu_ke_selesai, weeks, role_id }) {
    ...
    filtering = filtering || {};
        ...
        router.get('/laporan-mingguan', filtering);
    };
    ...
    return (
        <DashboardLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Laporan Mingguan</h2>}
        >
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6 bg-white border-b border-gray-200">
                            ...
                            <h1 className="text-2xl mb-5">Laporan Mingguan</h1>
                            ...
                            <div className="-mx-2 md:items-center md:flex mb-5">
                                ...
                                <div className="w-full sm:w-1/2 md:w-1/2 lg:w-1/4 xl:w-1/4 px-2">
                                    <label className="font-bold text-sm">Minggu Awal</label>
                                    <select name="minggu_ke_mulai" 
                                    defaultValue={minggu_ke_mulai} 
                                    className="w-full px-4 py-2 text-sm" onChange={(e) =>
                                        doFilterData("minggu_ke_mulai", e.target.value)
                                    }>
                                        <option value="">-Pilih Minggu-</option>
                                        {weeks.map((item, index) => (
                                            <option value={item.week}>{item.nama}</option>
                                        ))}
                                ...
                                <div className="w-full sm:w-1/2 md:w-1/2 lg:w-1/4 xl:w-1/4 px-2">
                                    <label className="font-bold text-sm">Minggu Akhir</label>
                                    <select name="minggu_ke_selesai" 
                                    defaultValue={minggu_ke_selesai} 
                                    className="w-full px-4 py-2 text-sm" onChange={(e) =>
                                        doFilterData("minggu_ke_selesai", e.target.value)
                                    }>
                                        <option value="">-Pilih Minggu-</option>
                                        {weeks.map((item, index) => (
                                            <option value={item.week}>{item.nama}</option>
                                            ...
                                            <tr>
                                                ...
                                                <td className="border px-4 py-2 text-sm">
                                                    <a
                                                        ...
                                                        href={route("lihat-laporan-mingguan", dt1.id_kelompok_siswa+"-"+id_semester+"-"+minggu_ke_mulai+"-"+minggu_ke_selesai)}
                                                        // href={route("laporan-mingguan", id_kelompok_siswa)}
                                                    >
                                                        Lihat
                                                    </a>
